...
The waste percentage (44.31\%) reflects the geometric overhead of circular coverage: the enclosing circle must extend beyond the convex hull at all points where the hull boundary is not tangent to the circle. Rural parishes in the northwest (Pacto, Gualea, Nanegal) and southeast (Pintag, Amaguaña) define the circle's diameter, while the hull follows the irregular parish distribution more closely.
...
\begin{itemize}
    ...
    \item About 44\% of the circular coverage area extends beyond the parish convex hull
    \item Multiple smaller antennas would likely be more efficient
\end{itemize}

The convex hull provides a tighter bound (2,765.79 km²), but real antenna coverage is inherently circular (or sector-based), making the enclosing circle the relevant constraint.
...
The Quito antenna network application demonstrates that these theoretical algorithms have direct practical value. By combining Welzl's algorithm with real geographic data, we determined that covering all 34 Quito parishes requires an antenna with 39.76 km radius, with 44\% area waste compared to the convex hull---highlighting the geometric overhead of circular coverage for irregularly distributed populations.
