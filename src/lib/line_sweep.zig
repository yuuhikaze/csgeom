...
fn segmentsIntersect(s1: Edge, s2: Edge) ?Point {
    ...
    // Calculate direction vectors using floats for precision
    const d1_x: f64 = @floatFromInt(p2.x - p1.x);
    const d1_y: f64 = @floatFromInt(p2.y - p1.y);
    const d2_x: f64 = @floatFromInt(p4.x - p3.x);
    const d2_y: f64 = @floatFromInt(p4.y - p3.y);

    // Calculate cross product (if 0, segments are parallel)
    const cross: f64 = d1_x * d2_y - d1_y * d2_x;
    if (@abs(cross) < 1e-10) return null; // Parallel segments

    // Calculate parametric intersection parameters
    const dx: f64 = @floatFromInt(p3.x - p1.x);
    const dy: f64 = @floatFromInt(p3.y - p1.y);

    const t: f64 = (dx * d2_y - dy * d2_x) / cross;
    const u: f64 = (dx * d1_y - dy * d1_x) / cross;

    // Check if intersection is within both segments [0, 1]
    if (t < 0.0 or t > 1.0 or u < 0.0 or u > 1.0) return null;

    // Calculate intersection point with proper rounding
    const p1_x: f64 = @floatFromInt(p1.x);
    const p1_y: f64 = @floatFromInt(p1.y);
    const ix: i32 = @intFromFloat(@round(p1_x + t * d1_x));
    const iy: i32 = @intFromFloat(@round(p1_y + t * d1_y));
